...
class Dastan:
    ...
    def __CalculatePieceCapturePoints(self, FinishSquareReference):
        # EDITED CODE STARTS
        piece = self._Board[self.__GetIndexOfSquare(FinishSquareReference)].GetPieceInSquare()
        if piece is not None:
            if self._CurrentPlayer.SameAs(self._Players[0]):
                self._Players[1].AddDeadPieces(piece)
            else:
                self._Players[0].AddDeadPieces(piece)
            return piece.GetPointsIfCaptured()
        # EDITED CODE ENDS
        return 0
        

    # ADDED CODE STARTS
    def __RestoreDead(self):
        pieces = self._CurrentPlayer.GetDeadPieces()
        if not pieces:
            print("No Pieces to Restore")
            return
        Choice = 0
        while Choice < 1 or Choice > len(pieces):
            for index,piece in enumerate(pieces):
                print(f"{index+1}) {piece.GetSymbol()}")
            Choice = int(input("Choose a piece to restore: "))
        square = random.choice(self._Board)
        while square.GetPieceInSquare() != None:
            square = random.choice(self._Board)
        square.SetPiece(pieces[Choice-1])
    # ADDED CODE ENDS
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3), 6 to restore dead pieces or 9 to take the offer: "))
                if Choice == 9:
...
class Player:
    ...
    def GetDeadPieces(self, index=-1):
        if index != -1:
            return self.__DeadPieces[index]
        return self.__DeadPieces

    def AddDeadPieces(self, piece):
        if piece != None:
            self.__DeadPieces.append(piece)
