...
class Dastan:
    ...
    def __init__(self, R, C, NoOfPieces):
        ...
        self.ChooseBackline(0) # ADDED CODE
        self.ChooseBackline(1) # ADDED CODE
        self.__CreateBoard()
    ...
    def __CheckSquareIsValid(self, SquareReference, StartSquare):
        if not self.__CheckSquareInBounds(SquareReference):
            return False
        PieceInSquare = self._Board[self.__GetIndexOfSquare(
            SquareReference)].GetPieceInSquare()
        if PieceInSquare is None:
            if StartSquare:
                return False
            else:
    ...
    def __CheckIfGameOver(self):
        ...
        for S in self._Board:
            ...
            if PieceInSquare is not None:
                if S.ContainsKotla() and PieceInSquare.GetTypeOfPiece(
                ) == "mirza" and not PieceInSquare.GetBelongsTo().SameAs(
                        S.GetBelongsTo()):
                    return True
                elif PieceInSquare.GetTypeOfPiece(
                ) == "mirza" and PieceInSquare.GetBelongsTo().SameAs(
                        self._Players[0]):
    ...
    def __GetSquareReference(self, Description):
        SelectedSquare = int(
            input("Enter the square " + Description +
    ...
    def __CalculatePieceCapturePoints(self, FinishSquareReference):
        if self._Board[self.__GetIndexOfSquare(
                FinishSquareReference)].GetPieceInSquare() is not None:
            return self._Board[self.__GetIndexOfSquare(
                FinishSquareReference)].GetPieceInSquare().GetPointsIfCaptured(
                )
        return
    ...
    def __UpdateBoard(self, StartSquareReference, FinishSquareReference):
        self._Board[self.__GetIndexOfSquare(FinishSquareReference)].SetPiece(
            self._Board[self.__GetIndexOfSquare(
                StartSquareReference)].RemovePiece())
    ...
    #CODE ADDED
    def ChooseBackline(self, player):
        SquareIsValid = False
        print(f"{self._Players[player].GetName()}: ")
        while not(SquareIsValid):
            squareReference = self.__GetSquareReference("to place your Kotla and Mirza")
            squareIndex = self.__GetIndexOfSquare(squareReference)
            if (player == 0 and squareIndex < self._NoOfColumns) or (
                    player == 1 and squareIndex > self._NoOfColumns *
                (self._NoOfRows - 1)):
                SquareIsValid = False
            else:
                self._Players[player].SetKotlaSquare(squareIndex)
                SquareIsValid = True
    #CODE ADDED
    ...
    def __CreateBoard(self):
        for Row in range(1, self._NoOfRows + 1):
            for Column in range(1, self._NoOfColumns + 1):
                if self.__GetIndexOfSquare(int(f"{Row}{Column}")) == self._Players[0].GetKotlaSquare():  # CODE EDITED
                    S = Kotla(self._Players[0], "K")
                elif self.__GetIndexOfSquare(int(f"{Row}{Column}")) == self._Players[1].GetKotlaSquare():  # CODE EDITED
    ...
    def __CreatePieces(self, NoOfPieces):
        for Count in range(1, NoOfPieces + 1):
            ...
            self._Board[self.__GetIndexOfSquare(2 * 10 + Count +
                                                1)].SetPiece(CurrentPiece)
        CurrentPiece = Piece("mirza", self._Players[0], 5, "1")
        self._Board[self._Players[0].GetKotlaSquare()].SetPiece(CurrentPiece) # CODE EDITED
        for Count in range(1, NoOfPieces + 1):
            CurrentPiece = Piece("piece", self._Players[1], 1, '"')
            self._Board[self.__GetIndexOfSquare((self._NoOfRows - 1) * 10 +
                                                Count +
                                                1)].SetPiece(CurrentPiece)
        CurrentPiece = Piece("mirza", self._Players[1], 5, "2")
        self._Board[self._Players[1].GetKotlaSquare()].SetPiece(CurrentPiece) # CODE EDITED
    ...
    def __CreateMoveOptions(self):
        self._Players[0].AddToMoveOptionQueue(
            self.__CreateMoveOption("ryott", 1))
        self._Players[0].AddToMoveOptionQueue(
            self.__CreateMoveOption("chowkidar", 1))
        self._Players[0].AddToMoveOptionQueue(
            self.__CreateMoveOption("cuirassier", 1))
        self._Players[0].AddToMoveOptionQueue(
            self.__CreateMoveOption("faujdar", 1))
        self._Players[0].AddToMoveOptionQueue(
            self.__CreateMoveOption("jazair", 1))
...
class MoveOption:
    ...
    def CheckIfThereIsAMoveToSquare(self, StartSquareReference,
                                    FinishSquareReference):
        ...
        for M in self._PossibleMoves:
            if StartRow + M.GetRowChange(
            ) == FinishRow and StartColumn + M.GetColumnChange(
            ) == FinishColumn:
                return True
        return False
...
class Player:
    ...
    def __init__(self, N, D):
        ...
        self.__KotlaSquare = 0 # ADDED CODE
    ...
    def GetScore(self):
        return self.__Score
    # ADDED CODE STARTS
    def GetKotlaSquare(self):
        return self.__KotlaSquare

    def SetKotlaSquare(self, square):
        self.__KotlaSquare =  square
    # ADDED CODE END
    ...
    def CheckPlayerMove(self, Pos, StartSquareReference,
                        FinishSquareReference):
        Temp = self.__Queue.GetMoveOptionInPosition(Pos - 1)
        return Temp.CheckIfThereIsAMoveToSquare(StartSquareReference,
                                                FinishSquareReference)
