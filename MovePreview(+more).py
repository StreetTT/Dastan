...
class Dastan:
    def __init__(self, R, C, NoOfPieces):
        self._Board = []
        self._Players = [Player("Player One", 1), Player("Player Two", -1)]
        self._Moves = {
            "jazair": ((2, 0), (2, -2), (2, 2), (0, 2), (0, -2), (-1, 1), (-1, 1)),
            "cuirassier": ((1, -2), (1, 2), (1, 0), (2, 0)),
            "chowkidar": ((1, 1), (1, -1), (-1, 1), (-1, -1), (0, 2), (0, -2)),
            "ryott": ((0, 1), (0, -1), (1, 0), (-1, 0)),
            "faujdar": ((0, 1), (0, -1), (0, 2), (0, -2))
        }
        self._MoveOptionOffer = list(self._Moves.keys())
    ...
    def __PreviewMove(self, position, startSquareRef):
        ...
        for Count in range(1, self._NoOfColumns + 1):
            print("---", end="")
        print("-")
        tempMoveOption = self.__CreateMoveOption(self._CurrentPlayer.GetQueue().GetMoveOptionInPosition(position - 1).GetName(),
                                self._CurrentPlayer.GetDirection())
        for Row in range(1, self._NoOfRows + 1):
            print(str(Row) + " ", end="")
            for Column in range(1, self._NoOfColumns + 1):
                print("|", end="")
                if tempMoveOption.CheckIfThereIsAMoveToSquare(startSquareRef,(Row * 10 + Column)):
                    print(" x", end="")
                else:
                    print("  ", end="")       
            print("|")
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                ...
                if Choice == 9 and not(OfferUsed):
                    self.__UseMoveOptionOffer()
                    OfferUsed = True
                    self.__DisplayState()
                elif OfferUsed:
                    print("Offer has been used for this turn")
            ...
            SquareIsValid = False
            self.__PreviewMove(Choice,StartSquareReference)
            while not SquareIsValid:
    ...
    def __CreateMoveOption(self, Name, Direction):
        NewMoveOption = MoveOption(Name)
        for i in range(len(self._Moves[Name])):
            NewMove = Move(self._Moves[Name][i][0] * Direction,
                           self._Moves[Name][i][1] * Direction)
            NewMoveOption.AddToPossibleMoves(NewMove)
        return NewMoveOption

    def __CreateMoveOptions(self):
        for i in range(len(self._Players)):
            for j in range(len(self._MoveOptionOffer)):
                self._Players[i].AddToMoveOptionQueue(
                    self.__CreateMoveOption(self._MoveOptionOffer[j],
                                            self._Players[i].GetDirection()))
class Piece:
