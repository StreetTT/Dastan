...
class Dastan:
    ...
    def __UndoMove(self,PrevMove):
        self._CurrentPlayer = PrevMove[2]
        self.__UpdateBoard(PrevMove[1], PrevMove[0])
        CurrentScore = self._CurrentPlayer.GetScore()
        self._CurrentPlayer.ChangeScore(-(self._CurrentPlayer.GetScore()) + PrevMove[3] )
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 0 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3), 9 to take the offer or 0 to undo the previous turn: "))
                if Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()
                if Choice == 0:
                    PreviousMove = self._MoveHistory.pop()
                    if PreviousMove == False:
                        Choice = -1
                        print("No previous move made")
            if Choice == 0:
                self.__UndoMove(PreviousMove)
            else:
                PreviousMove = []
                ...
                PreviousMove.append(StartSquareReference)
                SquareIsValid = False
                while not SquareIsValid:
                ...
                PreviousMove.append(FinishSquareReference)
                PreviousMove.append(self._CurrentPlayer)
                PreviousMove.append(self._CurrentPlayer.GetScore())
                ...
                self._MoveHistory.push(PreviousMove) #Add info to Queue
