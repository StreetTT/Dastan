...
class Dastan:
    ...
    def __GetSquareReference(self, Description):
        ...
        if not (valid):
            ...
            valid = self.__GetValidInt(SelectedSquare,1)
    ...
    def __UseMoveOptionOffer(self):
        ...
        while not(valid):
            ...
            valid = self.__GetValidInt(ReplaceChoice,2)
            if valid:
    ...
    def __CalculatePieceCapturePoints(self, FinishSquareReference):
      ...
      # START OF EDIT
    def __GetValidInt(self, userInput, type=0):
        try:
            _ = int(userInput)
        except:
            ...
            return False
        if type == 1:
            if len(userInput) != 2 or int(userInput[0]) > (self._NoOfRows) or int(userInput[1]) > (self._NoOfColumns):
                print("Enter a refernce to a valid square")
                return False
        elif type == 2:
            if int(userInput) > 5 or int(userInput) < 1:
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                while not(valid):
                    ...
                    valid = self.__GetValidInt(Choice)
                    if valid:
                        Choice = int(Choice)
                  # END OF EDIT
                if Choice == 9:
