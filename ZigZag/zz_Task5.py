    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3), 9 to take the offer and 8 to peak at next player's queue: "))
                if Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()
                elif Choice == 8:
                    PointsForPieceCapture = PointsForPieceCapture - 5
            ...
            if MoveLegal:
                PointsForPieceCapture = PointsForPieceCapture + self.__CalculatePieceCapturePoints(FinishSquareReference)
             #  END OF EDIT
