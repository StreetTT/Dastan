...
class Dastan:
    ...
    def __ModifyQueueOptions(self):
        ...
        while Choice < 1 or Choice > 5:    
            ...
            Choice = input("Choose a modify option to use from queue (1 to 5): ")
            ...
            try:
                ...
                self._CurrentPlayer.ChangeScore(3)
            if Choice == 1:
                self._CurrentPlayer.ReversePlayerQueue()
            elif Choice == 2:
                if self._CurrentPlayer.SameAs(self._Players[0]):
                    OppQueue = self._Players[0].GetMoveOptionQueue()
                else:
                    OppQueue = self._Players[1].GetMoveOptionQueue()
                self._CurrentPlayer.ReplaceQueue(OppQueue)
            elif Choice == 3:
                self._CurrentPlayer.SwapFirstAndLast()
            elif Choice == 4:
                MoveToFrontChoice = int(input("Choose the move option from your queue to move to the front (1 to 5): "))
                self._CurrentPlayer.MoveItemToFront(MoveToFrontChoice)
            elif Choice == 5:
                self._CurrentPlayer.ChangeScore(3)
            else:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3), 6 to modify your queue, or 9 to take the offer: "))
                if Choice == 9:
...
class MoveOptionQueue:
    ...
    def MoveItemToFront(self,pos):
        Temp = [self.__Queue.pop(pos)]
        self.__Queue = Temp + self.__Queue
...
class Player:
    ...
    def MoveItemToFront(self,pos):
        self.__Queue.MoveItemToFront(pos-1)

    def ReplaceQueue(self,queue):
        self.__Queue = queue

    def GetMoveOptionQueue(self):
        return self.__Queue
