...
class Dastan:
    ...
    def __AwardWafr(self):
        if not (self._Players[0].GetWafrAwarded() or self._Players[1].GetWafrAwarded()):
            WhoGetsWafr = random.randint(0, 3)
            if WhoGetsWafr in (0,1):
                self._Players[WhoGetsWafr].SetWafrAwarded()

    def PlayGame(self):
        GameOver = False
        while not GameOver:
            ChoiceUpperBoundry = 3
            self.__DisplayState()
            self.__AwardWafr()
            if self._CurrentPlayer.GetWafrAwarded():
                print("You have been awarded a Wafr, you can select any move from your queue for free this turn")
                ChoiceUpperBoundry = 5
            SquareIsValid = False
            Choice = 0
            while Choice < 1 or Choice > ChoiceUpperBoundry:
                Choice = int(input(f"Choose move option to use from queue (1 to {ChoiceUpperBoundry}) or 9 to take the offer: "))
                if Choice == 9 and not(self._CurrentPlayer.GetWafrAwarded()):
...
class Player:
    ...
    def SetWafrAwarded(self):
        self.__WafrAwarded = not(self.__WafrAwarded)
