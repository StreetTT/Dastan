...
class MoveOptionQueue:
    ...
    def GetQueueAsString(self):
        ...
        for i in range(len(self.__Queue)):
            QueueAsString += str(Count) + ". " + self.__Queue[(self.front + i) % 5].GetName() + "   "
            Count = (Count+1)
        return QueueAsString

    def Add(self, NewMoveOption):
        self.rear = (self.rear+1)%5
        self.__Queue[(self.rear)] = (NewMoveOption)
        

    def Replace(self, Position, NewMoveOption):
        self.__Queue[(self.front+Position)%5] = NewMoveOption

    def MoveItemToBack(self, Position):
        Temp = self.__Queue[(self.front+Position)%5]
        self.__Queue[(self.front+Position)%5] = self.__Queue[(self.rear)]
        self.__Queue[(self.rear)] = Temp

    def GetMoveOptionInPosition(self, Pos):
        return self.__Queue[(Pos+self.front)%5]
